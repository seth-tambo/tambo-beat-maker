...
export function TransportBar() {
  ...
  return (
    <>
      <div
        className="fixed top-4 left-4 z-50 flex items-center gap-2 px-3 py-2 rounded-xl bg-black/60 backdrop-blur-md border border-white/10"
        style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)" }}
      >
        {/* Play / Pause */}
        <button
          ...
          style={{
            ...
            cursor: !hasPads && !isPlaying ? "not-allowed" : "pointer",
          }}
          onClick={async () => {
          ...
          title={!hasPads && !isPlaying ? "Add pads to play" : isPlaying ? "Pause" : "Play"}
        >
          {isPlaying ? (
            <svg width="18" height="18" viewBox="0 0 24 24" fill={TRANSPORT_COLOR.glow}>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            style={{ color: CONTROL_COLOR.glow, background: "rgba(255,255,255,0.06)" }}
            onClick={() => setBpm(bpm - 5)}
          >
            &minus;
          </button>
          <div className="flex flex-col items-center min-w-[40px]">
            <span className="text-[9px] uppercase tracking-widest" style={{ color: `${CONTROL_COLOR.glow}70` }}>
              BPM
            </span>
            <span className="text-sm font-bold tabular-nums leading-none" style={{ color: CONTROL_COLOR.glow }}>
              {bpm}
            </span>
          </div>
          <button
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={SHARE_COLOR.glow} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
