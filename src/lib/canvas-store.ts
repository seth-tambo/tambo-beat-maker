...
function cloneHistorySnapshot(snapshot: HistorySnapshot): HistorySnapshot {
    return {
        ...
        padNotes: new Map(
            Array.from(snapshot.padNotes.entries(), ([padId, notes]) => [
                padId,
                new Set(notes)
            ])
        ),
...
function applyHistorySnapshot(snapshot: HistorySnapshot) {
    const nextOpenRollPadId =
        ...
        snapshot.pads.some((pad) => pad.id === snapshot.openRollPadId)
            ? snapshot.openRollPadId
            : null;
    padCounter = snapshot.padCounter;
    state = {
...
export function addPad(
    pad: Omit<BeatPad, 'id' | 'size' | 'muted'> & {
        id?: string;
        size?: number;
        muted?: boolean;
    }
...
export function setBpm(bpm: number) {
    const nextBpm = Math.max(30, Math.min(300, Math.round(bpm / 5) * 5));
    ...
    if (state.isPlaying && state.playStartedAt) {
        ...
        nextStartStep =
            (state.playStartedAtStep + elapsed * prevStepsPerMs) % STEPS;
    } else {
...
export function hydrateFromSoundboard(data: SoundboardData) {
    ...
    const fixedPads = data.pads.map((p) => {
        ...
        const aligned =
            Math.abs(p.x - expected.x) < CELL / 4 &&
            Math.abs(p.y - expected.y) < CELL / 4;
